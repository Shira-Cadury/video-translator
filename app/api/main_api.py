    ...
    try:
        ...
        try:
            cmd = [
                ...
                '-vf', f"subtitles={paths['srt_lang']}:force_style='FontSize=20,PrimaryColour=&H00FFFF,OutlineColour=&H000000,BorderStyle=1'",
                '-c:a', 'copy', burned_video_path
            ]
            subprocess.run(cmd, check=True)
            final_video_url = burned_video_path
        except Exception as ffmpeg_err:
            logger.error(f"FFmpeg failed: {ffmpeg_err}")
            final_video_url = raw_video_path    
...
@app.get("/health")
def health():
    return {"status": "ok", "model_loaded": transcription_service.model is not None}
